...
def _update_covered_reference(covered, i, j, coeff):
...
def get_covered_reference(u, v):
    ...
    for i in xrange(h):
        for j in xrange(w):
            ...
            y_dst = i+int(myf)
            x_dst = j+int(mxf)
            _update_covered_reference(covered, y_dst,   x_dst  , (1.-cy)*(1.-cx))
            _update_covered_reference(covered, y_dst+1, x_dst  , (   cy)*(1.-cx))
            _update_covered_reference(covered, y_dst  , x_dst+1, (1.-cy)*(   cx))
            _update_covered_reference(covered, y_dst+1, x_dst+1, (   cy)*(   cx))
